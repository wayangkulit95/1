...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome maeribo vpn service ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/wayngkulit95/1/main/REGIST | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/wayngkulit95/1/main/REGIST | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/wayngkulit95/1/main/REGIST | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/wayngkulit95/1/main/"
...
function base_package() {
    clear
    ########
    print_install "Menginstall Packet Yang Diperlukan"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/wayngkulit95/1/main/REGIST | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/wayngkulit95/1/main/REGIST | grep $ipsaya | awk '{print $3}')
...
<code>────────────────────</code>
<i>Automatic Notification Installation Scrpt</i>
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ🐳","url":"https://t.me/maeribo"},{"text":"ɪɴꜱᴛᴀʟʟ🐬","url":"https://t.me/maeribo"}]]}'
...
#update
# set time GMT +8
ln -fs /usr/share/zoneinfo/Asia/Kuala_Lumour /etc/localtime
